...
###Function###
echo "Running this script will detect the seven housekeeping genes included in the public MLST scheme from PubMLST and reveal the ST type"
Analysis="MLST"
...
#Path to nucleotide reference database and ST type file
PATHNAME_MLST="$HOME/YourFolder/MLST_database.fasta"
PATHNAME_ST="$HOME/YourFolder/MLST_database/ST_definitions.txt"
#mlst_delimiter (to check in species.fasta file --> e.g. adk_1 --> delimiter = _
Deli="_"
...
################################################################################################################################################################
#MLST analysis in parallel
source `which env_parallel.bash`
env_parallel -j $Jobs --compress --tmpdir $PATH_temp 'srst2 --input_pe {} $(echo {} | sed "s/_R1$FastqType$/_R2$FastqType/1") --forward $Fw --reverse $Rv --log --mlst_db $PATHNAME_MLST --mlst_definitions $PATHNAME_ST --mlst_delimiter $Deli --gene_max_mismatch $GeneMaxMis --max_divergence $MaxDiv --min_coverage $MinCov --output $PATH_output/$(echo {/}| cut -d '_' -f 1) --use_existing_scores --use_existing_pileup' ::: $PATH_input/*$Fw$FastqType
...
srst2 --prev_output $PATH_output/*__mlst__*__results.txt --output $PATH_output/$Analysis

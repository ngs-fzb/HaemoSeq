...
while getopts ":hi:o:c:d:f:r:s:t:e::v" option; do #:h does not need argument, f: does need argument
   case $option in
      ...
      o) # 
         ...
         PATH_output=$PATH_output_tmp/PLSDB_SRST2 ;;
      d) # 
         db_PLSDB_SRST2=$OPTARG;;
      c) # 
         cpu=$OPTARG;;
      f) # 
         Fw=$OPTARG;;
...
if [[ -z "$PATH_fastQ" ]] || [[ -z "$PATH_output" ]] || [[ -z "$db_PLSDB_SRST2" ]]
then
	echo "Please provide all required arguments (-i PATH_fastQ, -o PATH_output and -d db_PLSDB_SRST2)! use starter_PLSDB_SRST2.sh -h for help on syntax"
	exit
fi
...
echo "Plasmid database used: "$db_PLSDB_SRST2 >> $PATH_output/info.txt
...
[[ ! -s $db_PLSDB_SRST2".rev.1.bt2" ]] && bowtie2-build $db_PLSDB_SRST2 $db_PLSDB_SRST2; samtools faidx $db_PLSDB_SRST2
...
env_parallel -j $cpu --compress --tmpdir $PATH_tmp 'srst2 --input_pe {} $(echo {} | sed "s/$Fw/$Rv/1") --forward $Fw --reverse $Rv --log --gene_db $db_PLSDB_SRST2 --gene_max_mismatch $GeneMaxMis --max_divergence $MaxDiv --min_coverage $MinCov --output $PATH_output/$(echo {/}| cut -d '_' -f 1) --use_existing_scores  --use_existing_bowtie2_sam --use_existing_pileup' ::: "${group_toDownload[@]}"
fi
...
###Naming output files###
Name_itolFile_Summary_plasmids=$set"_PLSDB-SRST2_plasmid_presence_itol.txt" #This file can be dragged and dropped over an itol tree to visualize the rpesence of plasmids
...
echo "DATASET_LABEL	PLSDB-SRST2_plasmid_presence" >> $PATH_output/$Name_itolFile_Summary_plasmids
#Dataset color (can be changed later)
echo "COLOR	#ff0000" >> $PATH_output/$Name_itolFile_Summary_plasmids
#Legend
echo "LEGEND_TITLE	PLSDB-SRST2_plasmid_presence">> $PATH_output/$Name_itolFile_Summary_plasmids

...
geneDB=""
#geneDB="/mnt/c/Users/diric/Desktop/Bashscripts2/Class_Haemophilus_and_Serotyping-v3.fasta" #Example Path
...
PATH_temp=""
#PATH_temp="$HOME/tmp" #Example Path
...
################################################################################################################################################################
echo "Start species identification and in silico serotyping"
#Species identification in parallel
source `which env_parallel.bash`
env_parallel -j $Jobs --compress --tmpdir $PATH_temp 'srst2 --input_pe {} --forward $Fw --log --gene_db $geneDB --gene_max_mismatch $GeneMaxMis --max_divergence $MaxDiv --min_coverage $MinCov --output $PATH_output/$(echo {/}| cut -d '_' -f 1) --use_existing_scores --use_existing_pileup' ::: $PATH_input/*$Fw$FastqType
echo "Done with species identification and in silico serotyping"
#Remove pileup and bam files, only keep result files
rm $PATH_output/*.pileup
rm $PATH_output/*.bam
rm $PATH_output/*.sam

echo "Start compiling results of all samples"
#Create a combined file
srst2 --prev_output $PATH_output/*__genes__*__results.txt --output $PATH_output/$Analysis

echo "Done compiling results of all samples"
echo "Start producing output files"
